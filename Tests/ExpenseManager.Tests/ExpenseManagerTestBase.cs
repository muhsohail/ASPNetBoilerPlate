...
{
    public abstract class ExpenseManagerTestBase : AbpIntegratedTestBase<ExpenseManagerTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected ExpenseManagerTestBase()
        {
        ...
        protected void UsingDbContext(Action<ExpenseManagerDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ExpenseManagerDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ExpenseManagerDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ExpenseManagerDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ExpenseManagerDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ExpenseManagerDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ExpenseManagerDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ExpenseManagerDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ExpenseManagerDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ExpenseManagerDbContext, Task<T>> func)
        {
            T result;
